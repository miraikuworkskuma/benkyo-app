...
- AIは **Gemini API(`gemini-3.5-flash`)**。各利用者が **自分の無料APIキー** を使う
...
運営者に課金されない。無料枠はFlash系が対象(Pro系は2026年4月から有料専用)。
**注意: モデルは提供終了で404になることがある**(2026-07-09にgemini-2.5-flashが提供終了→
2026-07-16に`gemini-3.5-flash`へ載せ替え済み)。404が出たら最新のFlash系モデルIDに更新する。
...
- **Gemini**: `generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent`(モデル名は`GEMINI_MODEL`定数)。
...
- Gemini無料枠はレート制限あり(1分あたり・1日の上限)。429は `callGemini` でハンドリング済み。モデル提供終了(404)も専用メッセージでハンドリング済み。
